...
export const swapTokens = async (
  ...
  // call the `icebearTokenToEth` function from the contract
  ...
  if (ethSelected) {
    ...
    await tx.wait();
    // call icebearTokenToEth function which would take in `swapAmountWei` of `Crypto Dev` tokens and would
    // send back `tokenToBeReceivedAfterSwap` amount of `Eth` to the user
    tx = await exchangeContract.icebearTokenToEth(
      swapAmountWei,
      tokenToBeReceivedAfterSwap
    );
  }
